...
def traer_Django():
	id_tmdb = idtmdb.get()
	existe = servitools.existe(id_tmdb) 
	
	if existe == "True":
	   name = servitools.traer_titulo(id_tmdb)
	   titulo.set(name)
	   
	   desc = servitools.traer_sinopsis(id_tmdb)
	   txtsinopsis.delete(1.0,END)
	   txtsinopsis.insert(END, desc)
	   
	   date = servitools.traer_estreno(id_tmdb)
	   estreno.set(date)
	   
	   FanArt = servitools.traer_fanart(id_tmdb)
	   fanart.set(FanArt)
	   
	   poster = servitools.traer_thumbnail(id_tmdb)
	   thumbnail.set(poster)
	   
	   genre = servitools.traer_genero(id_tmdb)
	   genero.set(genre)
	else:
	   name = "La pelicula no ha sido registrada en el servidor Django"
	   titulo.set(name)   

def modificar_DjangoDB():
	name = titulo.get()
	id_movie = idtmdb.get()
	existe = servitools.existe(id_movie) 
	if existe == "True" and id_movie != "" and name != "No se encontro Titulo":
		id_movie = idtmdb.get()
	   ...
	   print("Pelicula no existe en Django, No se modificara ningun dato") 
...
bntconsultarTMDB = Button(ventana1, text="Consulta Django", command=traer_Django)
...
bntmodificarDjangoDB = Button(ventana1, text="Modificar DjangoDB", command=modificar_DjangoDB)
bntmodificarDjangoDB.grid(row=6,column=2)
